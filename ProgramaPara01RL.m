...
errorDeEntrenamiento=errorDeEntrenamiento+abs(errorU);
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  for i=1:tamR
         tamipp=size(red{i,1});
         idhp=arregloPesos(i,1);
         for x=1:tamipp(1,1)
             for y=1:tamipp(1,2)
                fprintf(idhp,'%f\t',red{i,1}(x,y));
             end
         end
         fprintf(idhp,'\r\n');
         tamipb=size(bias{i,1});
         idhp=arregloBias(i,1);
         for x=1:tamipb(1,1)
             for y=1:tamipb(1,2)
         fprintf(idhp,'%f\t',bias{i,1}(x,y));
             end
         end
         fprintf(idhp,'\r\n');
         
  end
  cantidadDeActualizaciones=cantidadDeActualizaciones+1;
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%
if n_v==num_val
    fprintf('Se terminó sin un aprendizaje adecuado');
    quieresContinuar=2;
else
...
if quieresContinuar==1
    ...
    fprintf('Se terminó el aprendizaje de manera no satisfactoria\n');
end
else
    fprintf('Se terminó el aprendizaje de manera satisfactoria\n');
    quieresContinuar=2;
end
end
%%%
...
fclose(idiErrorV);
fclose(idiContadorV);
%cerramos todos los archivos del historial de pesos y bias
%%%%%%%%%%%%%%%%%%% abrimos los .txt para la grafica de pesos y bias

for iPB=1:tamR
    fclose(arregloPesos(iPB,1));

    fclose(arregloBias(iPB,1));
end
%%%%%%%%%%%%%%%%%%%para la grafica de pesos y bias
...
%propago hacia adelante
    ...
    plot(cV,eV,'+');
%
ar=zeros(cantidadDeActualizaciones,1);
for o=1:cantidadDeActualizaciones
    ar(o,1)=o;
end
%%%%%%%%%%%%%%%%%%% abrimos los .txt para la grafica de pesos y bias
fprintf('cantidad de actualizaciones:');
...
for iPB=1:tamR
    ...
    contadorD=1;
for iPB=1:tamR
    tamCapa=size(red{iPB,1});
    cantidadDePesos=tamCapa(1,1)*tamCapa(1,2);
    idpg=arregloPesos(iPB,1);
    formatSpecce='%f';
    size1=[cantidadDePesos,cantidadDeActualizaciones];
    pesos=fscanf(idpg,formatSpecce,size1);
    fclose(idpg);
    numCa=int2str(iPB);
    nombreP=strcat('Evolución de pesos capa: ',numCa);
    for io=1:cantidadDePesos
        subplot(tamR,2,contadorD),plot(ar(:,1),pesos(io,:)),title(nombreP);
        hold on
    end
    contadorD=contadorD+1;
    %
    
    tamCapa=size(bias{iPB,1});
    cantidadDeBias=tamCapa(1,1)*tamCapa(1,2);
    idpg=arregloBias(iPB,1);
    formatSpecce='%f';
    size1=[cantidadDeBias,cantidadDeActualizaciones];
    pbias=fscanf(idpg,formatSpecce,size1);
    fclose(idpg);
    numCa=int2str(iPB);
    nombreB=strcat('Evolución de bias capa: ',numCa);
    for io=1:cantidadDeBias
        subplot(tamR,2,contadorD),plot(ar(:,1),pbias(io,:)),title(nombreB);
        hold on
    end
     contadorD=contadorD+1;
end
